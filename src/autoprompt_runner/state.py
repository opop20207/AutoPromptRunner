...
This is the practical subset of the full PROJECT.md state machine used by the
persistence layer in this step: CREATED, RUNNING, DONE, FAILED, STOPPED. Transitions
are deliberately explicit and small; anything not listed is rejected with a
``ValueError`` so an invalid status change can never be written to the database.
"""
...
class RunStatus(str, Enum):
    """The run statuses persisted by this step."""
...
_ALLOWED_TRANSITIONS: Dict["RunStatus", Set["RunStatus"]] = {
    ...
    RunStatus.RUNNING: {RunStatus.DONE, RunStatus.FAILED, RunStatus.STOPPED},
    RunStatus.DONE: set(),
